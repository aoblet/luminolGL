...
bool imgui3Slider(const char* text, float* val, float vmin, float vmax, float vinc, int pos, bool enabled)
{
        g_state.widgetId++;
        unsigned int id = (g_state.areaId<<16) | g_state.widgetId;
        ...
        int x,y;   
        int w = g_state.widgetW/3-0.1; 
        int h = SLIDER_HEIGHT;
        if(pos==1){
            x = g_state.widgetX;
            y = g_state.widgetY - SLIDER_HEIGHT ;
        } 
        else if(pos==2){
            x = g_state.widgetX+w;
            y = g_state.widgetY + DEFAULT_SPACING;
            g_state.widgetY += SLIDER_HEIGHT + DEFAULT_SPACING;
        } 
        else if(pos==3){
            x = g_state.widgetX+w+w;
            y = g_state.widgetY + DEFAULT_SPACING; 
            g_state.widgetY += SLIDER_HEIGHT + DEFAULT_SPACING;       
        } 
...
bool imguiSliderInt(const char* text, int* val, float vmin, float vmax, float vinc, bool enabled)
{
        g_state.widgetId++;
        unsigned int id = (g_state.areaId<<16) | g_state.widgetId;
